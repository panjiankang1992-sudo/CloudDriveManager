"""Cloud download job manager — tracks PikPak offline download tasks with 30-minute watchdog timeout.

Manages the lifecycle of cloud download jobs:
  PENDING → DOWNLOADING → COMPLETED / FAILED / TIMEOUT

The watchdog thread runs every 60 seconds and marks any job that has been
in PENDING or DOWNLOADING state for more than 30 minutes as TIMEOUT.
"""
...
logger = get_logger("cloud_download_manager")

WATCHDOG_INTERVAL_SECS = 60
JOB_TIMEOUT_SECS = 30 * 60  # 30 minutes
...
class CloudDownloadJobManager:
    ...
    job older than 30 minutes as TIMEOUT.
    """
    ...
    def update_status(
        ...
        Valid statuses: pending, downloading, completed, failed, timeout
        """
        with self._lock:
    ...
    def load_pending_from_db(self) -> None:
        """Reload pending/downloading jobs from DB into memory on startup."""
        ...
        with self._lock:
            for row in rows:
                ...
                self._jobs[task_id] = job
        logger.info(f"Loaded {len(rows)} pending/downloading jobs from DB")
    ...
    def _check_timeouts(self) -> None:
        ...
        with self._lock:
            for task_id, job in self._jobs.items():
                if job.status in (_CloudDownloadState.PENDING, _CloudDownloadState.DOWNLOADING):
        ...
        for task_id in aged_task_ids:
            ...
            self.update_status(task_id, "timeout", "Task exceeded 30 minute timeout")
